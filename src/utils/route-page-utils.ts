import { CITIES, CITY_BELONGS_TO, CityName } from "../data/cicies";
import { EVENTS } from "../data/event";
import { findFatigue } from "../data/fatigue";
import { PRESTIGES } from "../data/prestige";
import { PRODUCTS } from "../data/products";
import { ROLE_RESONANCE_SKILLS } from "../data/roleSkills";
...
export const getOneGraphRecommendation = (
  ...
  if (!goAndReturn) {
    //console.log(fromCity,toCity,restock)
    const reco = buyCombinationsGo[fromCity]?.[toCity]?.[restock];
    //console.log(buyCombinationsGo[fromCity]?.[toCity]?.[restock])
    if (!reco) {
      return [];
    }
    return [reco];
